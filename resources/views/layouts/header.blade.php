              ...
              <div class="navbar-collapse" id="navbar-mobile">
                  <div class="mr-auto float-left bookmark-wrapper d-flex align-items-center">
                      ...
                      <ul class="nav navbar-nav bookmark-icons">
                          ...
                          <li class="nav-item d-none d-lg-block"><a class="nav-link" href="{{ route('chat.index') }}"
                                  data-toggle="tooltip" data-placement="top" title="Chat"><i
                                      class="ficon bx bx-chat"></i></a></li>
                         @if (auth()->user()->role=="1" || auth()->user()->role=="0")
                         <li class="nav-item d-none d-lg-block"><a class="nav-link" href="{{ route('admin.todo.index') }}"
                            data-toggle="tooltip" data-placement="top" title="Todo"><i
                                class="ficon bx bx-check-circle"></i></a></li>
                         @elseif(auth()->user()->role=="2")
                         <li class="nav-item d-none d-lg-block"><a class="nav-link" href="{{ route('b1.todo.index') }}"
                            data-toggle="tooltip" data-placement="top" title="Todo"><i
                                class="ficon bx bx-check-circle"></i></a></li>
                         @endif
                          <li class="nav-item d-none d-lg-block"><a class="nav-link" href="{{ url('/event') }}"
                      ...
                      <ul class="nav navbar-nav">
                          ...
                          <li class="nav-item d-none d-lg-block  align-self-center" ><span data-i18n="Apps">{{ auth()->user()->free_trial_days_left }} Days of Free Trial left.</span>
                          <a class="badge badge-light-danger" href="{{ route('upgrade.planner.plan') }}">Upgrade now.</a>
                          </li>
                          @endif
                          @if (auth()->user()->days_left <= 2 && auth()->user()->days_left > 0)
                          <li class="nav-item d-none d-lg-block  align-self-center" ><span data-i18n="Apps">{{ auth()->user()->days_left}} Days for your subscription left.</span>
                            <a class="badge badge-light-danger" href="{{ route('upgrade.planner.plan') }}">Upgrade now.</a>
                            </li>
                            @elseif(auth()->user()->days_left > 0)
                            <li class="nav-item d-none d-lg-block  align-self-center" ><span class="warning" data-i18n="Apps">{{ auth()->user()->days_left}} Days.</span>
                            </li>
                          @endif
                          @endif
                      </ul>
                  ...
                  <ul class="nav navbar-nav float-right">
                      ...
                      @if (auth()->user()->role=="2")
                      @php
                      $user_planner = \App\Models\BplannerVersion::where('user_id',auth()->user()->id)->with('planner')->first()->planner->version;
                      $admin_planner = \App\Models\Planner::latest()->first()->version;
                      @endphp
                      @if ($user_planner != $admin_planner)

                      <li class="nav-item d-grid  mtdev-1">
                          <div class="d-flex align-items-center ">
                              <span class="spinner-grow text-danger" style=" height: 15px; width: 15px; "></span>
                              <a class="text-primary" target="_blank"
                                  href="#" data-toggle="modal" data-target="#plannerupdate" >
                                  Update Your 3D Planner <span class="text-info">-></span>
                              </a>
                          </div>
                      </li>
                    
                      @endif
                      <li class="nav-item d-none d-lg-block mtdev-1">
                          <a  href="{{ route('planner.show', ['id'=>auth()->user()->company_name]) }}"
                              data-toggle="tooltip" data-placement="top" title="3D Planner">
                              <img width="45px" height="45px"
                                  src="https://img.icons8.com/external-xnimrodx-blue-xnimrodx/64/000000/external-3d-graphic-design-xnimrodx-blue-xnimrodx.png" />
                          </a>
                      </li>
                      @endif
                          ...
                          <ul class="dropdown-menu dropdown-menu-media dropdown-menu-right">
                              ...
                              <li class="scrollable-container media-list"><a class="d-flex justify-content-between"
                                      ...
                                      <div class="media d-flex align-items-center">
                                          <div class="media-left pr-0">
                                              <div class="avatar mr-1 m-0"><img
                                                      src="{{ asset('app-assets/images/portrait/small/avatar-s-4.jpg') }}"
                                                      alt="avatar" height="39" width="39"></div>
                                          </div>
                                          <div class="media-body">
                      ...
                      <li class="dropdown dropdown-user nav-item"><a class="dropdown-toggle nav-link dropdown-user-link"
                              ...
                              <div class="user-nav d-sm-flex d-none"><span class="user-name">@if (Auth::check())
                                      {{auth()->user()->name}}
                                      @endif</span><span class="user-status text-muted">Available</span></div><span><img
                                      class="round"
                                      @if(Auth::check()) src="{{ asset(auth()->user()->image_path) }}" @endif alt="avatar" onerror="this.onerror=null;this.src='{{URL::asset('app-assets/images/profile/user-uploads/error.jpg')}}';"
                                      height="40" width="40"></span>
                          </a>
                          <div class="dropdown-menu dropdown-menu-right pb-0"><a class="dropdown-item"
                                  href="{{ route('profile.edit') }}"><i class="bx bx-user mr-50"></i> Edit Profile</a><a
                                  class="dropdown-item" href="app-email.html"><i class="bx bx-envelope mr-50"></i> My
                                  Inbox</a>
                                  @if (auth()->user()->role=="1" || auth()->user()->role=="0")
                                  <a class="dropdown-item" href="{{ route('admin.todo.index') }}"><i
                                    class="bx bx-check-square mr-50"></i> Task</a>
                                  @elseif(auth()->user()->role=="2")
                                  <a class="dropdown-item" href="{{ route('b1.todo.index') }}"><i
                                    class="bx bx-check-square mr-50"></i> Task</a>
                                  @endif
                                
                                      <a class="dropdown-item"
                                  href="{{ route('chat.index') }}"><i class="bx bx-message mr-50"></i> Chats</a>
                              <div class="dropdown-divider mb-0"></div>
                              @if(session()->has("impersonate"))
                              <a class="dropdown-item" href="{{ route('impersonate.logout') }}"><i
                                      class="bx bx-power-off mr-50"></i> Stop Impersonate</a>
                              @endif
  ...
  <!-- END: Header-->
  @if (auth()->user()->role=="2")
   <!-- disabled plannerupdate Modal -->
<div class="modal text-left" id="plannerupdate" tabindex="-1" role="dialog" aria-labelledby="myModalLabel6" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myModalLabel6">3D Planner Verion {{$admin_planner}}</h4>
            ...
            <div class="modal-body">
                <p>
                  3D Version Planner Has Been Update We Have Many New Feature For You 
                </p>
                <div class="alert bg-rgba-success mb-0" role="alert">
                    <span class="text-bold-600">Verion {{$admin_planner}} !</span>
                    Are You Want To update Your Version
                </div>
            </div>
            <div class="modal-footer">
                ...
                <a href="{{ route('b1.planner.update') }}" class="btn btn-primary ml-1">
