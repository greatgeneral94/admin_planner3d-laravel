    ...
    <script>
        function Addtask(){
           ...
           // Send the data using post
            var posting = $.post( "{{ route('b1.todo.insert') }}",
            {
                "_token": "{{ csrf_token() }}",
                title: title,   
                assign_to: assign_to,   
        ...
        function updatetask(){
            ...
            var comment     = $(".comment-editor .ql-editor")[0].innerHTML;
           // Send the data using post
            var posting = $.post( "{{ route('b1.todo.update') }}",
            {
                "_token": "{{ csrf_token() }}",
                id: id,   
                title: title,   
        ...
        function favorite(id){
            var posting =   $.get("favorites/"+id, {},);
            posting.done(function( data ) {
                toastr.info("Task Status Updated");
        });
        }
        function trash(id){
            var posting =   $.get("trashs/"+id, {},);
            posting.done(function( data ) {
                toastr.info("Task Deleted");
        });
        }
        function completed(id){
            var posting =   $.get("completeds/"+id, {},);
            posting.done(function( data ) {
                toastr.info("Task Status Updated");
        });
        }
