...
$(function () {
  ...
  todoTaskListWrapper.on('click', '.todo-item', function (e) {
    ...
    var todocomment = $this.find(".todo-comment").text();
    var tododate = $this.find(".todo-date").text();
    var todoid = $this.find(".todo-id").text();
    taskTitle.val(todoTitle);
    var compose_editor = $(".compose-editor .ql-editor");
    compose_editor[0].innerHTML = tododescription;
    var comment_editor = $(".comment-editor .ql-editor");
    comment_editor[0].innerHTML = todocomment;
    $('.pickadate').val(tododate)
    $('.todo_id').val(todoid)
